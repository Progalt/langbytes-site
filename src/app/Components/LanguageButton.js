...
export function LanguageButton({ language, selectedLanguages, setSelectedLanguages }) {
    ...
    return (
        ...
        // </div>
        <button
            className={`bg-[#13131d]  py-1 w-32 rounded-xl text-sm border-2 hover:shadow-[0_0_20px_0] hover:shadow-indigo-500/50 transition-all duration-200 ${selectedLanguages.includes(language) ? "border-indigo-500" : "border-slate-800" }`}
            onClick={() => { 
                if (selectedLanguages.includes(language)) {
                    selectedLanguages = selectedLanguages.filter(item => item !== language);
                    setSelectedLanguages(selectedLanguages);
